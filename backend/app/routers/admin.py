...
@router.get("/suspicious-votes", response_model=dict)
async def get_suspicious_votes_simple(
    admin: User = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
    """
    Get all suspicious votes for dashboard (simplified response)
    """
    ...
    for vote in suspicious_votes:
        ...
        result.append({
            ...
            "created_at": vote.timestamp.isoformat() if vote.timestamp else None,
        })
...
async def handle_vote_action(
    ...
    elif action_request.action == "revalidate":
        if not vote.is_valid:
            vote.is_valid = True
            # Increment candidate vote count
            if candidate:
                candidate.vote_count += 1
            message = "Vote revalidated"
        else:
            message = "Vote was already valid"
        
    else:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid action. Use 'invalidate', 'clear_flag', or 'revalidate'"
        )
...
async def delete_candidate(
    ...
    - Requires admin role
    - Removes candidate and invalidates their votes
    """
    ...
    # Delete all votes for this candidate (cannot nullify candidate_id, must delete)
    votes = db.query(Vote).filter(Vote.candidate_id == candidate_id).all()
    vote_count = len(votes)
    for vote in votes:
        db.delete(vote)
    
    # Save candidate name before deletion
    ...
    activity = ActivityLog(
        ...
        details=f"Admin {admin.username} deleted candidate {candidate_name} and {vote_count} associated votes",
        ip_address="admin_panel"
    )
    db.add(activity)
